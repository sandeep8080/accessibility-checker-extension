...
async function getApiKey(): Promise<{
  ...
  const provider = (result.appSettings as AppSettings)?.ai.provider;
  const apiKey = (result.appSettings as AppSettings)?.ai.apiKeys[provider];
  return (result.appSettings as AppSettings)?.ai.apiKeys
    ? {
