...
import type { MappedViolation, AISuggestion, MessageAction, AIProvider } from '../types';

// In-memory cache fallback in case chrome.storage.session isn't available
const memoryCache = new Map<string, AISuggestion>();

async function getCachedSuggestion(cacheKey: string): Promise<AISuggestion | null> {
  try {
    const result = await chrome.storage.session.get(cacheKey);
    return (result[cacheKey] as AISuggestion) || memoryCache.get(cacheKey) || null;
  } catch {
    return memoryCache.get(cacheKey) || null;
  }
}

async function setCachedSuggestion(cacheKey: string, suggestion: AISuggestion): Promise<void> {
  try {
...
async function getApiKey(): Promise<{ key: string, provider: AIProvider } | null> {
  const result = await chrome.storage.local.get(['apiKey', 'aiProvider']);
  return result.apiKey ? { key: result.apiKey as string, provider: (result.aiProvider as AIProvider) || 'gemini' } : null;
}

// Generate suggestion using Google Gemini API
async function generateGeminiSuggestion(apiKey: string, violation: MappedViolation): Promise<AISuggestion> {
...
async function handleAISuggestion(violation: MappedViolation): Promise<AISuggestion> {
  ...
  if (!credentials) {
    throw new Error('API Key missing. Please configure your API key in the Settings tab.');
  }
